...
| Clean-runner CI exposed local-cache assumptions | Added workflow contract tests, made mobile verification build shared contracts first, moved Gradle initialization behind Expo's generated native-project boundary, and fixed visual fixtures to one clock/time zone with deterministic queue ordering. | Generated `android/` files stay out of source control, and screenshot tests compare layout rather than machine time or unstable equal-time ordering. |
