...
| Clean-runner CI exposed local-cache, emulator, and renderer assumptions | Added workflow contract tests, made mobile/APK verification build shared contracts first, configured KVM before Maestro, moved Gradle initialization behind Expo's native-project boundary, fixed test time/order, and reviewed separate macOS/Linux visual baselines. | Generated `android/` files stay out of source control, hardware acceleration avoids unreliable software-emulator timeouts, and each renderer keeps its own approved 3% visual gate. |
