...
| Clean-runner CI exposed local-cache and renderer assumptions | Added workflow contract tests, made mobile/APK verification build shared contracts first, moved Gradle initialization behind Expo's generated native-project boundary, fixed test time/order, and reviewed separate macOS/Linux visual baselines. | Generated `android/` files stay out of source control, and screenshot tests compare each renderer against its own approved output instead of weakening the 3% regression threshold. |
