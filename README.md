...
| Mobile CI assumed a checked-in Gradle project | Added a workflow contract test and moved Gradle initialization behind Expo's generated native-project boundary. | The repository keeps generated `android/` files out of source control; CI must not request a Gradle cache before `expo prebuild` creates its dependency files. |
